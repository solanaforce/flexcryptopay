...
from flexcryptopay.events import flexcryptopay_initialized
from flexcryptopay.models import BitcoinLightningInvoice as BLI, Setting
from flexcryptopay.modules.classes.crypto import Crypto
from flexcryptopay import db
from flexcryptopay.utils import format_decimal
from flexcryptopay.wallet_encryption import wallet_encryption
...
class BitcoinLightning(Crypto):
    ...
    def __init__(self) -> None:
        ...
        self.LIGHTNING_SEND_TO_FLEXCRYPTOPAY_PERIOD = int(
            environ.get("LIGHTNING_SEND_TO_FLEXCRYPTOPAY_PERIOD", 1)
        )
    ...
    def invoice_listener(self, app):
        app.logger.debug(f"waiting for flexcryptopay initialization...")
        flexcryptopay_initialized.wait()
        app.logger.debug(f"received flexcryptopay initialization signal!")
    ...
    def invoice_notificator(self, app):
        ...
        s = requests.Session()
        s.headers = {
            "X-Flexcryptopay-Backend-Key": environ.get(
                "FLEXCRYPTOPAY_BTC_BACKEND_KEY", "flexcryptopay"
            )
        }
        while True:
            sleep(self.LIGHTNING_SEND_TO_FLEXCRYPTOPAY_PERIOD)
            with app.app_context():
                try:
                    paid_invoices = BLI.query.filter(
                        (BLI.state == "SETTLED") & (BLI.sent_to_flexcryptopay == False)
                    ).all()
                    ...
                    for inv in paid_invoices:
                        ...
                        r = r.json()
                        if "success" == r["status"]:
                            inv.update(sent_to_flexcryptopay=True)
    ...
    def create_wallet(self):
        ...
        try:
            app.logger.debug(f"waiting for flexcryptopay encryption key...")
    ...
    def dump_wallet(self) -> Tuple[str, str]:
        ...
        filename = "flexcryptopay-btc-lightning-mnemonic-key.txt"
