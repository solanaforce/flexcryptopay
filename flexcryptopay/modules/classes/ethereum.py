...
class Ethereum(Crypto):
    ...
    def gethost(self):
        host = environ.get("ETHEREUM_API_SERVER_HOST", "ethereum-flexcryptopay")
        port = environ.get("ETHEREUM_SERVER_PORT", "6000")
        return f"{host}:{port}"

    def get_auth_creds(self):
        username = environ.get(f"ETH_USERNAME", "flexcryptopay")
        password = environ.get(f"ETH_PASSWORD", "flexcryptopay")
    ...
    def dump_wallet(self):
        ...
        filename = f"{now}_{self.crypto}_flexcryptopay_wallet.json"
