...
class Xrp(Ethereum):
    ...
    def gethost(self):
        host = environ.get("XRP_API_SERVER_HOST", "xrp-flexcryptopay")
        port = environ.get("XRP_SERVER_PORT", "6000")
        return f"{host}:{port}"

    def get_auth_creds(self):
        username = environ.get(f"XRP_USERNAME", "flexcryptopay")
        password = environ.get(f"XRP_PASSWORD", "flexcryptopay")
