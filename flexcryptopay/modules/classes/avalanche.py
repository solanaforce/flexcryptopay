...
class Avalanche(Ethereum):
    ...
    def gethost(self):
        host = environ.get("AVALANCHE_API_SERVER_HOST", "avalanche-flexcryptopay")
        port = environ.get("AVALANCHE_SERVER_PORT", "6000")
        return f"{host}:{port}"

    def get_auth_creds(self):
        username = environ.get(f"AVALANCHE_USERNAME", "flexcryptopay")
        password = environ.get(f"AVALANCHE_PASSWORD", "flexcryptopay")
