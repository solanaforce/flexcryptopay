...
from flexcryptopay.models import User, Wallet
...
def metrics_basic_auth(view):
    ...
    def wrapped_view(**kwargs):
        metrics_username = os.environ.get("METRICS_USERNAME", "flexcryptopay")
        metrics_password = os.environ.get("METRICS_PASSWORD", "flexcryptopay")
...
def login_required(view):
    ...
    def wrapped_view(**kwargs):
        if g.user is None:
            if "X-Flexcryptopay-Api-Key" in request.headers:
                return {
                    "status": "error",
                    "message": "This endpoint doesn't accept X-Flexcryptopay-Api-Key auth",
                }
...
def api_key_required(view):
    @functools.wraps(view)
    def wrapped_view(**kwargs):
        if "X-Flexcryptopay-Api-Key" not in request.headers:
            return {"status": "error", "message": "No API key"}

        apikey = request.headers["X-Flexcryptopay-Api-Key"]
