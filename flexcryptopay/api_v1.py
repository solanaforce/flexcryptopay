...
from flexcryptopay.auth import basic_auth_optional, login_required, api_key_required
...
from flexcryptopay.modules.cryptos.bitcoin_lightning import BitcoinLightning
from flexcryptopay.modules.cryptos.monero import Monero
from flexcryptopay.modules.rates import RateSource
from flexcryptopay.models import *
from flexcryptopay.callback import send_notification, send_unconfirmed_notification
from flexcryptopay.utils import format_decimal
from flexcryptopay.wallet_encryption import (
    wallet_encryption,
    WalletEncryptionPersistentStatus,
    WalletEncryptionRuntimeStatus,
)
from flexcryptopay.exceptions import NotRelatedToAnyInvoice
...
def payoutnotify(crypto_name):
    try:
        if "X-Flexcryptopay-Backend-Key" not in request.headers:
            app.logger.warning("No backend key provided")
            return {"status": "error", "message": "No backend key provided"}, 403

        crypto = Crypto.instances[crypto_name]
        bkey = environ.get(f"FLEXCRYPTOPAY_BTC_BACKEND_KEY", "flexcryptopay")
        if request.headers["X-Flexcryptopay-Backend-Key"] != bkey:
...
def backup(crypto_name):
    ...
    bkey = environ.get(f"FLEXCRYPTOPAY_BTC_BACKEND_KEY")
    req = requests.get(url, stream=True, headers={"X-FLEXCRYPTOPAY-BACKEND-KEY": bkey})
    headers = Headers()
